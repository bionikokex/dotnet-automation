...
Архитектура следует форме **root DSL → domain DSL → DriverStack → driver → page/api wrappers**:
...
| **DriverStack** | Предоставляет пару (UIDriver, ApiDriver) |
| **Driver** (`UIDriver`, `ApiDriver`) | Содержит actor, управляет lifecycle |
| **Page/API wrappers** | Низкоуровневая механика поверх Playwright |

### Ключевые свойства

- **DSL не хранит селекторы и routes** — локаторы находятся в page object как class-level constants, endpoints вынесены в отдельные классы
...
- **Общая DSL-инфраструктура** — `DslBase` находится в `Infrastructure/Dsl`, домены DSL — в `Dsl/`, инфраструктура runtime — в `Infrastructure/`.
...
- Локаторы находятся как class-level constants в соответствующих классах
